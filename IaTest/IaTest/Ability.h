...
public:
	Ability(string id,string name,string description,Effect firstEffect,int level=1,unsigned int classmask=ALL,float delay=0.5);
	~Ability();
...
Ability::Ability(std::string id, std::string name, std::string description, Effect firstEffect, int level,
				 unsigned int classmask, float delay){
    //this.effects = new vector();
	this->id = id;
	this->name = name;
	this->description = description;
	this->effects.push_back(firstEffect);
	this->level = level;
...
void Ability::setDelay(float delay){
	this->delay = delay;
}
